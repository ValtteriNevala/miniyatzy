...
export default function Gameboard () {
  ...
  function getDiceColor (i) {
    return selectedDices[i] ? 'grey' : 'red' // noppien väri, valittu : ei valittu
  }

    //tarkistetaan väri
  function getResultColor (i) {
    return selectedResult[i] ? 'grey' : 'red' // nappien väri, valittu : ei valittu
  }

  //noppien valitseminen
  ...
  //tuloksen valitseminen
  function selectResult (i) {
    //jos painettiin nappia, jossa on jo tulos
    if (finalResults[i] !== false) {
      console.log('Pisteet tallennettu jo!')
      setStatus("Valitse pisteille paikka , jota et ole vielä käyttänyt!")
    }
    //muuten
    else {
      ...
      setFinalSum(tulos)
      let vaille = BONUSPOINT - tulos
      setBonuspoints(vaille)
      setNbrOfThrowsLeft(NBR_OF_THROWS)
      setDisabledThrowButton(false)
  ...
  useEffect(() => {
    ...
    if (selectedResult.every(x => x)) {
      ...
      if (bonuspoints <= 0) {
        ...
        setText('Bonuksesta jäi puuttumaan ' + bonuspoints + ' pistettä.')
        setStatus('Peli päättyi, lopulliset pisteesi ovat: ' + finalSum)
      }
    }
  }, [nbrOfThrowsLeft])
  ...
  return (
    <View>
      <Pressable style={styles.restart} onPress={() => reset()}>
        <Text style={styles.restartButtonText}>Aloita uusi peli!</Text>
      </Pressable>
